...
fn kernel_add(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let y = src0[global_id.x + tensor_params.offs_src1/4u];
...
void Start() {
    ...
    wgpu::BindGroupLayoutEntry bgLayoutEntries[4] = {0};
    {
    ...
    ctx->bind_group_layout = ctx->device.CreateBindGroupLayout(&bindGroupLayoutDescriptor);
    ...
    {
        wgpu::PipelineLayoutDescriptor desc0{
            .label = "ggml-wgpu-pipeline-layout",
            .bindGroupLayoutCount = 1,
            .bindGroupLayouts = &ctx->bind_group_layout,
        };
        ctx->pipeline_layout = ctx->device.CreatePipelineLayout(&desc0);
        ...
        wgpu::ComputePipelineDescriptor desc1{
            ...
            .compute = wgpu::ProgrammableStageDescriptor{
                ...
                .entryPoint = "kernel_add",
            },
        };
        ctx->pipeline_add = ctx->device.CreateComputePipeline(&desc1);
...
void Alloc() {
    ...
    // alloc buffers
    {
        wgpu::BufferDescriptor desc0{
            ...
            .size = MIN_STORAGE_BUFFER_ALIGNMENT*16u,
...
void Compute() {
    ...
    size_t offs_B = 1u*MIN_STORAGE_BUFFER_ALIGNMENT;
    ctx->queue.WriteBuffer(ctx->buffer, offs_B, demo_mat_B, demo_mat_nbytes);
    size_t offs_dest = 2u*MIN_STORAGE_BUFFER_ALIGNMENT;
    //size_t offs_params = 3u*MIN_STORAGE_BUFFER_ALIGNMENT;
    ...
    auto & bgEntries = ctx->bgEntries;
    {
        ...
        bgEntries[0].size = MIN_STORAGE_BUFFER_ALIGNMENT;

        bgEntries[1].binding = 1;
        bgEntries[1].buffer = ctx->buffer;
        bgEntries[1].offset = offs_B;
        bgEntries[1].size = MIN_STORAGE_BUFFER_ALIGNMENT;

        bgEntries[2].binding = 2;
        bgEntries[2].buffer = ctx->buffer;
        bgEntries[2].offset = offs_dest;
        bgEntries[2].size = MIN_STORAGE_BUFFER_ALIGNMENT;

        bgEntries[3].binding = 3;
        bgEntries[3].buffer = ctx->buf_tensor_params;
        bgEntries[3].offset = offs_params;
        bgEntries[3].size = MIN_STORAGE_BUFFER_ALIGNMENT;
    }
    wgpu::BindGroupDescriptor bindGroupDescriptor{
        .label = "bind_group",
        .layout = ctx->bind_group_layout,
        .entryCount = 4,
        .entries = ctx->bgEntries,
    };
    auto bindGroup = ctx->device.CreateBindGroup(&bindGroupDescriptor);
    GGML_ASSERT(bindGroup);

    // compute
    wgpu::CommandEncoderDescriptor desc0{
...
static void handle_buffer_map(WGPUBufferMapAsyncStatus status, void * ready) {
    printf("buffer_map status=%#.8x\n", status);
    if (status == WGPUBufferMapAsyncStatus_Success) {
        bool * _ready = (bool *) ready;
        *_ready = true;
    }
}
...
void GetResult(void * output) {
    ...
    // command encoder
    wgpu::CommandEncoderDescriptor desc1{
    ...
    ctx->queue.Submit(1, &cmdBuffer);
    bool ready = false;
    tmpbuf.MapAsync(wgpu::MapMode::Read, 0, demo_mat_nbytes, handle_buffer_map, &ready);
    while (!ready) {
...
int main() {
    ...
    GetAdapter(ctx, [](wgpu::Adapter a) {
        ctx->adapter = a;
        GetDevice(ctx, [](wgpu::Device d) {
            ctx->device = d;
            Start();
            Alloc();
            Compute();
